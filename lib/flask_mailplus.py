...
from ceelo.extensions import mail


def send_template_message(template=None, ctx=None, *args, **kwargs):
    """
    Send a templated e-mail using similar signature as Flask-Mail:
    http://pythonhosted.org/Flask-Mail/

    Except, it also supports template rendering. If you want to use a template
    then just omit the body and html kwargs to Flak-Mail and instead supply
    a path to a template. It will auto-look and render the text/html messages.

    Example:
        ctx = {'user': current_user, 'reset_token':, token}
        end_template_message('Password reset from Foo', ['you@example.com'],
...
def _try_render_template(template_path, ext='txt', **kwargs):
    ...
    do a pth exists based on a relative path to the template.
